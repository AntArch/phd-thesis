...
class ImageAnalysis():      
    ...
    def hist_compare(self):
        ...
        for band in range(self.bands):
            ...
            plt.plot(x_vals,hist_arch_norm, color='#3A01DF')
            #plt.fill_between(x_vals,hist_arch_norm, color='#3A01DF',alpha=0.5)
            plt.plot(x_vals,hist_back_norm, color='#FF8000')
            #plt.fill_between(x_vals,hist_back_norm, color='#FF8000', alpha=0.5)
            plt.xlabel('Normalised distance')
            plt.ylabel('Normalised frequency')
            plt.title(band_names[band], fontweight='bold')
            plt.axis('tight')
            plt.savefig('hist'+str(band)+'.png')
            plt.close()
            
            sum_of_difference = np.sum(np.abs(hist_arch_norm-hist_back_norm))
                                              
            print sum_of_difference       
            print band_names[band],band
        ...
        plt.xlim([0,6])
        plt.barh(pos,emd_comp, color='#3A01DF')
        plt.margins(0.01)
        
        plt.yticks(pos+0.5, band_names, style ='italic')
        
        plt.xlabel("Contrast (Earth Mover's Distance)",fontweight='bold')
        #plt.ylabel('RGB')
